...
# `it-XX` branch in StefanMaron/MSDyn365BC.Code.History.
...
#   2. Find the highest-numbered upstream it-XX branch
...
BRANCH_PREFIX="it-"  # Prefix for IT branches

echo "🚀 Starting Business Central IT branch sync process (v${SCRIPT_VERSION})"
...
# Fetch all it-* branches from upstream as remote-tracking branches
echo "Fetching it-* branches from upstream..."
git fetch --quiet upstream "+refs/heads/${BRANCH_PREFIX}*:refs/remotes/upstream/${BRANCH_PREFIX}*"
echo "✓ Upstream it-* branches fetch completed"

# Debug: Show what branches we got from upstream
echo "Available upstream it-* branches:"
git for-each-ref --format='  %(refname:short)' "refs/remotes/upstream/${BRANCH_PREFIX}*" || echo "  No it-* branches found"

# --------------------------------------------------------------------
# 2) Find the highest-numbered upstream branch (e.g. it-26)
# --------------------------------------------------------------------
echo ""
echo "=== Step 2: Finding highest-numbered upstream branch (v${SCRIPT_VERSION}) ==="

# List all remote-tracking it-* branches from upstream, strip the prefix,
# sort numerically by the number after it-, and pick the highest one.
...
#    We parse the latest commit message for 'it-XX' and 'SHA: xxxxxxx'.
# --------------------------------------------------------------------
echo ""
echo "=== Step 3: Checking current version and SHA on main (v${SCRIPT_VERSION}) ==="

# Check if main exists, then extract the it-XX version and SHA from the latest commit message.
...
if git rev-parse --verify -q refs/heads/main >/dev/null; then
  ...
  if [[ -n "$sync_commit_msg" ]]; then
    ...
    # New format: "Sync to upstream it-26 (SHA: abc123)"
    # Old format: "Sync to upstream upstream/it-26"
